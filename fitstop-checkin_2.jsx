...
// ─── STORAGE HELPERS ─────────────────────────────────────────────────────────
const STORAGE_KEY = "fitstop-checkins-v1";
...
export default function App() {
  ...
  const [view, setView] = useState("prep"); // prep | recap | history
  const [tab, setTab] = useState("focus");  // focus | talking | notes
  ...
  async function archive() {
    const snap = {
      date: draft.date, type: draft.type, sentiment: draft.sentiment,
      recapNotes: draft.recapNotes, actions: [...draft.actions], members: draft.members,
    };
    ...
    await save(reset);
    setView("history");
    showToast("Check-in archived ✓");
  }

  // Circleback search via Anthropic API with MCP
  async function cbSearch() {
    ...
    try {
      ...
      try {
        const meetings = JSON.parse(txt.replace(/```json|```/g, "").trim());
        setCbMeetings(Array.isArray(meetings) ? meetings : []);
      } catch { setCbMeetings([]); }
    } catch { setCbMeetings([]); }
  ...
  async function cbImport() {
    ...
    } catch { showToast("Import failed — enter notes manually"); }
  ...
  function updateAction(i, key, val) {
    setDraft(d => { const actions = d.actions.map((a, x) => x === i ? { ...a, [key]: val } : a); return { ...d, actions }; });
  }
  ...
  if (loading) return (
    <div style={{ background: "#111", height: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ ...D, color: "#F26722", fontSize: 14, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase" }}>Loading…</div>
    </div>
  );
  ...
  return (
    <div style={{ ...S, background: "#111", color: "#fff", height: "100vh", display: "flex", flexDirection: "column", overflow: "hidden", position: "relative" }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
              ...
              <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px" }}>
                ...
                {tab === "talking" && (
                  <>
                    ...
                    <button onClick={addTP} style={{ background: "transparent", border: "0.5px dashed rgba(255,255,255,0.15)", borderRadius: 4, color: "#888", fontFamily: "'Barlow Condensed', sans-serif", fontSize: 11, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", padding: "7px 14px", cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 6 }}>
                      + Add point
                    </button>
                  </>
                )}
          ...
          {view === "recap" && (
            <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px" }}>
              ...
              <button onClick={addAction} style={{ background: "transparent", border: "0.5px dashed rgba(255,255,255,0.15)", borderRadius: 4, color: "#888", ...D, fontSize: 11, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", padding: "7px 14px", cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 6 }}>
                + Add action item
              </button>
            </div>
          )}
      ...
      <div style={{ background: "#1A1A1A", borderTop: "1px solid rgba(255,255,255,0.08)", padding: "12px 24px", display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
        {view === "prep" && <>
          <button onClick={saveAndToast} style={{ background: "#F26722", border: "none", borderRadius: 4, color: "#fff", ...D, fontSize: 13, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", padding: "9px 20px", cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>💾 Save Prep</button>
        ...
        {view === "recap" && <>
          <button onClick={saveAndToast} style={{ background: "#F26722", border: "none", borderRadius: 4, color: "#fff", ...D, fontSize: 13, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", padding: "9px 20px", cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>💾 Save Recap</button>
        ...
        {view === "history" && <>
          <button onClick={() => setView("prep")} style={{ background: "#F26722", border: "none", borderRadius: 4, color: "#fff", ...D, fontSize: 13, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", padding: "9px 20px", cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>+ New Check-In</button>
        </>}
        <span style={{ fontSize: 12, color: "#888", marginLeft: "auto" }}>
          Fitstop {currentLoc?.name}, {currentLoc?.state}{currentLoc?.presales ? " · Presales" : ""}
        </span>
      </div>

      {/* TOAST */}
      {toast && (
        <div style={{ position: "fixed", bottom: 24, right: 24, background: "#1A1A1A", border: "0.5px solid rgba(242,103,34,0.4)", borderRadius: 6, padding: "12px 16px", fontSize: 13, color: "#fff", display: "flex", alignItems: "center", gap: 10, zIndex: 100, boxShadow: "0 4px 24px rgba(0,0,0,0.4)" }}>
          <span style={{ color: "#F26722" }}>✓</span> {toast}
        </div>
      )}
    </div>
  );
